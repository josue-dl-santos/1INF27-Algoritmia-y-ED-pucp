 ...
 * Author: alulab14
 *
 * Created on 5 de abril de 2025, 10:11 AM
 */
...
bool VerificaPresupuestoYSeg(int *cromosoma,Recurso *recursos,int presupuesto,
        int &costo,int seguridad,int &nivelseg){
    int costoparcial=0;
    ...
    for(int i=0;i<11;i++){
        ...
        nseg+=recursos[i].nivelSeg;
        }
    }
    if(costoparcial>=costominimo and costoparcial<=presupuesto and nseg>=seguridad){
        costo=costoparcial;
        nivelseg=nseg;
        return true;
    }
...
void imprimeCromo(int *cromosoma,int combinacion,int costo,int nseg){
...
//        cout<<cromosoma[i];
//    }
    cout<<"| Costo: "<<costo<<" miles | Nivel de Seguridad: "<<nseg<<endl;
}
void CalculaCombinaciones(int combinaciones,Recurso *recursos,int presupuesto,
    ...
    for(int i=0;i<combinaciones;i++){
        ...
        if(VerificaRecursos(cromosoma,recursos))
            if(VerificaPresupuestoYSeg(cromosoma,recursos,presupuesto,costo,
                    nivelseguridad,nseg)){
                bandera=true;
                imprimeCromo(cromosoma,i,costo,nseg);
            }
                
    }
    if(!bandera)
        cout<<"No se encontraron combinaciones que cumplan con lo solicitado";
    
}
void ingresoDeDatos(int &nivelseguridad,int &presupuesto){
    int pre,seg;
...
int main(int argc, char** argv) {
    ...
    ingresoDeDatos(nivelseguridad,presupuesto);
    CalculaCombinaciones(combinaciones,recursos,presupuesto,nivelseguridad);
    return 0;
}
